...
To process these exported policies (it should create 3 xml files in the folder specified), run the script on a machine which has teh RASAdmin PowerShell module but it does not need to be run with a user that has RAS permissions
...
The script will error if CreateRASPoliciesFromCitrix.ps1 already exists and -overwrite is not specified. Theoutput file name can be changed with -outputFile
