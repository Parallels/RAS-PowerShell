...
function Get-RasGuestObjectForCloneAwareFlow {
    ...
    if ($cloneTrackingCompleted) {
        ...
        Write-DebugLog "CLONE-AWARE FLOW: tracking removed for VM [$VmId] after successful clone completion."
    }
    else {
    ...
    try {
        ...
        foreach ($vm in $clusterVMs) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($vmId)) {
                ...
                $isAutoName = -not [string]::IsNullOrWhiteSpace($vmName) -and
                    $vmId -match '^\d+$' -and
...
function Handle-TaskInfo {
    ...
    try {
        ...
        if ($null -ne $ctx -and $ctx.ContainsKey('type') -and [string]$ctx.type -eq 'clone') {
            ...
            Write-DebugLog "Clone task [$taskId]: tracking removed for clone VM [$cloneId] after successful completion."
