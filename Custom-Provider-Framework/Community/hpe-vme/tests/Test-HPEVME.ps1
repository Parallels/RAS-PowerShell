<#
.SYNOPSIS
    End-to-end test for the HPE VM Essentials custom provider.
.DESCRIPTION
    Drives Parallels-RAS-CFP-HPE-VME.ps1 through the shared CustomProvider.psm1
    ...
    Authenticate with -Token, or with -Username/-Password. If none are given, the
    script falls back to a CustomProvider.psd1 found next to this script or in the
    repository root.
.EXAMPLE
    pwsh -File .\Test-HPEVME.ps1 -Url https://vme.example.com -Token $env:VME_TOKEN
.EXAMPLE
    pwsh -File .\Test-HPEVME.ps1 -Url https://vme.example.com -Username admin -Password $env:VME_PASS -GuestID 42 -TestSnapshots
#>
...
$ProviderDir = Split-Path $PSScriptRoot -Parent           # hpe-vme/
$RepoRoot    = Split-Path $ProviderDir -Parent            # repository root

$ProviderScript = Join-Path $ProviderDir 'Parallels-RAS-CFP-HPE-VME.ps1'
...
function Resolve-CustomSettings {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Url) -and
        (-not [string]::IsNullOrWhiteSpace($Token) -or
         (-not [string]::IsNullOrWhiteSpace($Username) -and -not [string]::IsNullOrWhiteSpace($Password)))) {
        $s = @{ url = $Url; skip_tls = $SkipTls }
        if (-not [string]::IsNullOrWhiteSpace($Token)) { $s.token = $Token }
        if (-not [string]::IsNullOrWhiteSpace($Username)) { $s.username = $Username; $s.password = $Password }
        return $s
    }
    ...
    throw 'Provide -Url plus -Token (or -Username/-Password), or a CustomProvider.psd1 with CustomSettings.'
}
...
function Invoke-Pipeline {
    ...
    Write-Host "  ip_addresses=$(@($guest.ip_addresses) -join ',')"
