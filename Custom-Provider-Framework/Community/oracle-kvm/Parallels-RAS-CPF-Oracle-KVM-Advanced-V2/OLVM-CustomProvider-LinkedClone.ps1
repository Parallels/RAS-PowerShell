...
$writer = New-Object System.IO.StreamWriter($stdout, [System.Text.UTF8Encoding]::new($false))
$writer.AutoFlush = $true

function Get-RuntimeSetting {
    param([object]$Root, [string[]]$Path, [object]$Default)

    $current = $Root
    foreach ($part in $Path) {
        if ($null -eq $current -or $null -eq $current.PSObject.Properties[$part]) { return $Default }
        $current = $current.$part
    }
    if ($null -ne $current -and $null -ne $current.PSObject.Properties['value']) { return $current.value }
    return $current
}

function Resolve-RuntimePath {
    param([string]$DataDirectory, [string]$ConfiguredPath, [string]$DefaultName)

    $path = if ([string]::IsNullOrWhiteSpace($ConfiguredPath)) { $DefaultName } else { $ConfiguredPath }
    if ([System.IO.Path]::IsPathRooted($path)) { return $path }
    return Join-Path $DataDirectory $path
}

$script:ProviderRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$script:SettingsPath = if (-not [string]::IsNullOrWhiteSpace($env:OLVM_RAS_SETTINGS_PATH)) {
    $env:OLVM_RAS_SETTINGS_PATH
} else {
    Join-Path $script:ProviderRoot 'RAS-CPF-OLVM-Settings.json'
}
$script:RuntimeSettings = $null
if (Test-Path -LiteralPath $script:SettingsPath) {
    $script:RuntimeSettings = Get-Content -LiteralPath $script:SettingsPath -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
}

$dataDirectory = [string](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('locations', 'data_directory') -Default 'C:\CFP Scripts')
$script:ProviderNamePrefix = 'OLVM:'
$script:LogPath = Resolve-RuntimePath -DataDirectory $dataDirectory -ConfiguredPath ([string](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('locations', 'log_file') -Default 'OLVM-RAS-Provider.log')) -DefaultName 'OLVM-RAS-Provider.log'
$script:TaskStatePath = Resolve-RuntimePath -DataDirectory $dataDirectory -ConfiguredPath ([string](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('locations', 'task_state_file') -Default 'OLVM-RAS-TaskState.json')) -DefaultName 'OLVM-RAS-TaskState.json'
$script:TemplateFlagStatePath = Resolve-RuntimePath -DataDirectory $dataDirectory -ConfiguredPath ([string](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('locations', 'template_state_file') -Default 'OLVM-RAS-TemplateFlagState.json')) -DefaultName 'OLVM-RAS-TemplateFlagState.json'
$script:LinkedTemplateStatePath = Resolve-RuntimePath -DataDirectory $dataDirectory -ConfiguredPath ([string](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('locations', 'linked_template_state_file') -Default 'OLVM-RAS-LinkedTemplateState.json')) -DefaultName 'OLVM-RAS-LinkedTemplateState.json'
$script:OlvmSession = $null
$script:TaskContext = @{}
$script:TaskMaxAgeMinutes = [int](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('tasks', 'max_age_minutes') -Default 30)
$script:SnapshotPersistMemory = [bool](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('snapshots', 'persist_memory') -Default $false)
$script:ExcludedVmNames = @((Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('inventory', 'excluded_vm_names') -Default @('HostedEngine')))
...
function Test-TaskExpired {
    ...
    try {
        ...
        $age = (Get-Date).ToUniversalTime() - $created.ToUniversalTime()
...
function Get-LinkedTemplateKey {
    param([string]$VmId, [string]$SnapshotName)
    return "$VmId|$SnapshotName"
}

function Get-LinkedTemplateState {
    return Read-JsonStateFile -Path $script:LinkedTemplateStatePath
}

function Get-LinkedTemplate {
    param([string]$VmId, [string]$SnapshotName)
    $state = Get-LinkedTemplateState
    $key = Get-LinkedTemplateKey -VmId $VmId -SnapshotName $SnapshotName
    if (-not $state.ContainsKey($key)) { return $null }
    return $state[$key]
}

function Set-LinkedTemplate {
    param([string]$VmId, [string]$SnapshotName, [string]$TemplateId, [string]$TemplateName)
    Invoke-WithStateLock -LockName 'OlvmRasLinkedTemplateStateLock' -Action {
        $state = Get-LinkedTemplateState
        $state[(Get-LinkedTemplateKey -VmId $VmId -SnapshotName $SnapshotName)] = @{
            id = $TemplateId; name = $TemplateName
        }
        Save-JsonStateFile -Path $script:LinkedTemplateStatePath -State $state
    }
}

function Remove-LinkedTemplateMapping {
    param([string]$VmId, [string]$SnapshotName)
    Invoke-WithStateLock -LockName 'OlvmRasLinkedTemplateStateLock' -Action {
        $state = Get-LinkedTemplateState
        $state.Remove((Get-LinkedTemplateKey -VmId $VmId -SnapshotName $SnapshotName)) | Out-Null
        Save-JsonStateFile -Path $script:LinkedTemplateStatePath -State $state
    }
}

function Get-LatestRasSnapshotName {
    param([string]$VmId)
    $names = @(Get-OlvmVmSnapshots -VmId $VmId |
        Where-Object { (Test-HasProperty $_ 'description') -and [string]$_.description -match '^RAS_TEMPLATE_VERSION_\d+$' } |
        ForEach-Object { [string]$_.description })
    return $names | Sort-Object { [int]($_ -replace '^RAS_TEMPLATE_VERSION_', '') } | Select-Object -Last 1
}
...
function Invoke-OlvmRestMethod {
    ...
    try {
        ...
        Write-DebugLog "HTTP failure ($statusCode): $($_.Exception.Message)$(if ($responseBody) { ' - Response: ' + $responseBody })"
...
function Handle-Initialize {
    return @{
        result = @{
            version      = '1.0.0'
            capabilities = @{
                can_suspend_guests    = [bool](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('capabilities', 'can_suspend_guests') -Default $true)
                guests_polling_rate   = [int](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('capabilities', 'guests_polling_rate') -Default 15)
                tasks_polling_rate    = [int](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('capabilities', 'tasks_polling_rate') -Default 5)
                tasks_polling_retries = [int](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('capabilities', 'tasks_polling_retries') -Default 360)
                template_method       = [string](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('capabilities', 'template_method') -Default 'versioning')
                can_link_clones       = [bool](Get-RuntimeSetting -Root $script:RuntimeSettings -Path @('capabilities', 'can_link_clones') -Default $true)
            }
        }
    }
}
...
function Get-AllGuestAndTemplateIds {
    ...
    foreach ($vm in (Get-OlvmVms)) {
        if ((Test-HasProperty $vm 'id') -and
            (-not (Test-HasProperty $vm 'name') -or [string]$vm.name -notin $script:ExcludedVmNames)) {
            $ids += [string]$vm.id
        }
    }
...
function Handle-GuestConvert {
    ...
    try {
        ...
        if ($isTemplate) {
            $snapshotName = Get-LatestRasSnapshotName -VmId $vmId
            if ([string]::IsNullOrWhiteSpace($snapshotName)) {
                throw 'No RAS template-version snapshot exists for the VM'
            }

            $existing = Get-LinkedTemplate -VmId $vmId -SnapshotName $snapshotName
            if ($null -ne $existing -and (Test-OlvmObjectExists -Probe { Get-OlvmTemplate -TemplateId ([string]$existing.id) })) {
                $taskId = New-TaskId
            ...
            $templateName = "RASLT_$($vmId.Substring(0, 8))_$($snapshotName -replace '^RAS_TEMPLATE_VERSION_', 'v')_$([guid]::NewGuid().ToString('N').Substring(0, 6))"
            $template = Invoke-OlvmApi -Method POST -Path 'templates' -Body @{ name = $templateName; vm = @{ id = $vmId } }
            if ($null -eq $template -or -not (Test-HasProperty $template 'id')) {
                throw 'OLVM template creation did not return an id'
            }

            $taskId = New-TaskId
            Set-TaskStateEntry -TaskId $taskId -Entry @{
                type = 'template_create'; template_id = [string]$template.id; template_name = $templateName
                vm_id = $vmId; snapshot_name = $snapshotName; created_utc = (Get-Date).ToUniversalTime().ToString('o')
            }
            return @{ result = @{ task_id = $taskId } }
        }
...
function Handle-GuestClone {
    ...
    try {
        ...
        $isLinkClone = (Test-HasProperty $Params 'is_link_clone') -and [bool]$Params.is_link_clone
        ...
        if ($isLinkClone) {
            $linkedTemplate = Get-LinkedTemplate -VmId $sourceId -SnapshotName $snapshotName
            if ($null -eq $linkedTemplate) {
                throw "No immutable OLVM template is registered for snapshot '$snapshotName'"
            }
            $body.template = @{ id = [string]$linkedTemplate.id }
        }
        elseif ($isTemplateSource) {
            ...
            if ($snapshotName) {
                ...
                # Snapshot creation doesn't automatically carry this setting
                # into AddVmFromSnapshot, and Windows 11 is rejected without it.
                $sourceVm = Invoke-OlvmApi -Method GET -Path "vms/$sourceId`?all_content=true"
                if (Test-HasProperty $sourceVm 'tpm_enabled') {
                    $body.tpm_enabled = [bool]$sourceVm.tpm_enabled
                }
            }
        }

        Write-DebugLog "Cloning source [$sourceId] -> name [$cloneName] snapshot [$snapshotName], linked [$isLinkClone]"
        $path = if ($isTemplateSource -and -not $isLinkClone) { 'vms?clone=true' } else { 'vms' }
        $newVm = Invoke-OlvmApi -Method POST -Path $path -Body $body
...
function Handle-GuestSnapshotsCreate {
    ...
    try {
        ...
        if ($null -ne $existing) {
            Remove-LinkedTemplateMapping -VmId $vmId -SnapshotName $snapshotName
            ...
            Set-TaskStateEntry -TaskId $taskId -Entry @{
                ...
                vm_id           = $vmId
                old_snapshot_id = [string]$existing.id
                snapshot_name   = $snapshotName
                created_utc     = (Get-Date).ToUniversalTime().ToString('o')
            }
            Write-DebugLog "Snapshot [$snapshotName] deletion submitted for replacement on guest [$vmId], task [$taskId]"
            return @{ result = @{ task_id = $taskId } }
        }

        $snap = Invoke-OlvmApi -Method POST -Path "vms/$vmId/snapshots" -Body @{
            description         = $snapshotName
            persist_memorystate = $script:SnapshotPersistMemory
        }
...
function Handle-GuestSnapshotsDelete {
    ...
    try {
        ...
        # ponytail: retain the immutable OLVM template because existing linked
        # clones depend on it; add reference-counted cleanup if storage demands it.
        Remove-LinkedTemplateMapping -VmId $vmId -SnapshotName $snapshotName
...
function Handle-TaskInfo {
    ...
    try {
        ...
        switch ($type) {
            ...
            'template_create' {
                $templateId = [string]$ctx.template_id
                try { $template = Get-OlvmTemplate -TemplateId $templateId }
                catch { return @{ result = @{ state = 'running' } } }

                $status = if (Test-HasProperty $template 'status') { [string]$template.status } else { 'unknown' }
                if ($status -eq 'ok') {
                    Set-LinkedTemplate -VmId ([string]$ctx.vm_id) -SnapshotName ([string]$ctx.snapshot_name) -TemplateId $templateId -TemplateName ([string]$ctx.template_name)
                    Remove-TaskStateEntry -TaskId $taskId
                    return @{ result = @{ state = 'completed'; output = @{ template_id = $templateId } } }
                }
                if ($status -eq 'locked') { return @{ result = @{ state = 'running' } } }
                Remove-TaskStateEntry -TaskId $taskId
                return @{ result = @{ state = 'failed'; error = @{ code = 1; message = "Template status [$status]" } } }
            }

            'snapshot_replace' {
                $vmId = [string]$ctx.vm_id
                $oldSnapId = [string]$ctx.old_snapshot_id
                if (Test-OlvmObjectExists -Probe { Invoke-OlvmApi -Method GET -Path "vms/$vmId/snapshots/$oldSnapId" }) {
                    return @{ result = @{ state = 'running' } }
                }

                $snap = Invoke-OlvmApi -Method POST -Path "vms/$vmId/snapshots" -Body @{
                    description         = [string]$ctx.snapshot_name
                    persist_memorystate = $script:SnapshotPersistMemory
                }
                if ($null -eq $snap -or -not (Test-HasProperty $snap 'id')) {
                    throw "Replacement snapshot creation did not return a snapshot id."
                }

                $ctx.type = 'snapshot_create'
                $ctx.snapshot_id = [string]$snap.id
                Set-TaskStateEntry -TaskId $taskId -Entry $ctx
                Write-DebugLog "Replacement snapshot [$($ctx.snapshot_name)] creation submitted for guest [$vmId], snapshot id [$($snap.id)], task [$taskId]"
                return @{ result = @{ state = 'running' } }
            }
            ...
            'clone' {
                ...
                # OLVM finishes a full clone in the powered-off state. RAS owns
                # the subsequent power-on, guest-agent and RDP readiness steps.
                if ($guest.state -ne 'powered_off') {
                    return @{ result = @{ state = 'running' } }
                }
                ...
                Write-DebugLog "Clone task [$taskId]: VM [$vmId] is powered off and ready for RAS. Completing."
...
function Process-Method {
    ...
    $safeInputLine = $InputLine -replace '("password"\s*:\s*)"(?:\\.|[^"\\])*"', '$1"***"'
    Write-DebugLog "IN: $safeInputLine"
