...
function Invoke-Pipeline {
    ...
    Invoke-AsyncTask $IOStreams $capbilities.tasks_polling_rate -ScriptBlock {
        return Submit-GuestsConvert -IOStreams $IOStreams -GuestID $GuestID -IsTemplate $false
    } | Out-Null

    if ((Submit-GuestsGet $IOStreams $GuestID).is_template) {
        throw "$GuestID is still a template"
    }

    if ($snapshotName) {
        Invoke-AsyncTask $IOStreams $capbilities.tasks_polling_rate -ScriptBlock {
            return Submit-GuestsSnapshotsRevert $IOStreams $GuestID $snapshotName
        } | Out-Null
    }

    if ("powered_on" -ne (Submit-GuestsGet $IOStreams $GuestID).state) {
        Submit-GuestsControl $IOStreams $GuestID "start" | Out-Null
    }

    while ("powered_on" -ne (Submit-GuestsGet $IOStreams $GuestID).state) {
