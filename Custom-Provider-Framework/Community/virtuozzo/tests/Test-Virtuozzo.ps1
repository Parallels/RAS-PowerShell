<#
.SYNOPSIS
    End-to-end test for the Virtuozzo Hybrid Infrastructure custom provider.
.DESCRIPTION
    Drives Parallels-RAS-CFP-Virtuozzo.ps1 through the shared CustomProvider.psm1
    ...
    Connection settings come from -AuthUrl/-Username/-Password/-Project, or fall
    back to a CustomProvider.psd1 found next to this script or in the repo root.
.EXAMPLE
    pwsh -File .\Test-Virtuozzo.ps1 -AuthUrl https://vhi.example.com:5000/v3 -Username admin -Password $env:VHI_PASS -Project vdi
.EXAMPLE
    pwsh -File .\Test-Virtuozzo.ps1 -AuthUrl https://vhi.example.com:5000/v3 -Username admin -Password $env:VHI_PASS -Project vdi -GuestID <server-id> -TestSnapshots
#>
...
$ProviderDir = Split-Path $PSScriptRoot -Parent           # virtuozzo/
$RepoRoot    = Split-Path $ProviderDir -Parent            # repository root

$ProviderScript = Join-Path $ProviderDir 'Parallels-RAS-CFP-Virtuozzo.ps1'
...
function Resolve-CustomSettings {
    if (-not [string]::IsNullOrWhiteSpace($AuthUrl) -and
        ...
        if (-not [string]::IsNullOrWhiteSpace($Region)) { $s.region = $Region }
        if (-not [string]::IsNullOrWhiteSpace($CloneNetworkId)) { $s.clone_network_id = $CloneNetworkId }
        return $s
    }
    ...
    throw 'Provide -AuthUrl, -Username, -Password and -Project, or a CustomProvider.psd1 with CustomSettings.'
}
