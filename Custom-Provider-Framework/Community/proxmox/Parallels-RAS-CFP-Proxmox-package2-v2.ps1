...
function Wait-ProxmoxTaskCompletion {
    param(
        [Parameter(Mandatory = $true)]
        [string]$TaskId,
    ...
    while ($true) {
        ...
        $result = New-TaskResultState -TaskStatus $taskStatus
        ...
        throw "Task [$TaskId] failed: $($result.error.message)"
    }
}

function Remove-AllProxmoxVmSnapshots {
    param(
        [Parameter(Mandatory = $true)]
        [string]$Node,
    ...
    foreach ($snapshot in $snapshots) {
        if ($snapshot.PSObject.Properties.Name -contains 'name' -and -not [string]::IsNullOrWhiteSpace([string]$snapshot.name)) {
            ...
            $deleted += @{ name = $snapshotName; task_id = $taskId }
        }
    }
...
function Handle-GuestConvert {
    ...
    try {
        ...
        if ($isTemplate) {
            # Proxmox cannot convert a VM to a template while snapshots remain.
            # If RAS created a version snapshot before conversion, remove it first.
            $snapshots = Get-ProxmoxVmSnapshots -Node $node -VmId $vmId
            if ($null -ne $snapshots -and $snapshots.Count -gt 0) {
                Write-DebugLog "VM [$vmId] has $($snapshots.Count) snapshot(s); removing snapshots before template conversion."
                Remove-AllProxmoxVmSnapshots -Node $node -VmId $vmId | Out-Null
            }
