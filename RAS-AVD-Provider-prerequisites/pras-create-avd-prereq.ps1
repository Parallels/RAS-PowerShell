...
.NOTES 
    ...
    Changelog:  0.1 -   Initial version
                0.2 -   Install NuGet if needed, import Az.Resources if needed, remove duplicate secrets,
                        reordered ouput, Output is reordered to match AVD Provider wizard in console, error handling,
                        validating input, only showing locations where AVD is supported, User input validation checks,
                        specify vnet and set permissions
                0.3     Bug fix setting correct Graph API permission ID
                0.4     Explicitly Import az.network
                0.5     Added option to create Azure Keyvault and store App registration client secret. Upon launch of script,
                        check for internet connectivity and DNS, and check if the OS is not based on ARM as importing modules
                        does not work in that case.
                0.6     Bug fix in Azure Location selection, added missing newline when no vnet is found, added functionality
                        to also provide the app registration VM Reader permissions on subscription level to allow listing Azure VM
                        when using standalone hosts or custom templates.
                1.0     Updates to descriptions and error checks after final proof read
#> 
